...
class Tracer_Mod(object):
    ...
    def Tracer_Tracking(self, prec, evap, temp, ConP, ConAvg, ConSuAvg,  p, alpha_Su, alpha_SG, **kwargs):
       ...
       #variable DEFINITION
        ...
        sr, rs, ea, rff, rfs, pe, cr,  alphat  = self.get_root_zone_balance(pr, pe, ep, srmax=p[0], lp=p[1], Rsmax=p[2], beta=p[3], dt=dt, cp = p[11], alpha_Su = alpha_Su,  Ptresh=p[15])
 ...
 ### Fuctions to be used in the model
    ...
    def get_root_zone_balance(pr, pe, ep, srmax=250.0, lp=0.25, Rsmax=2.0, beta=2.0, dt=1.0, cp = 0.8, alpha_Su = 1.0, Ptresh = 10):
        ...
        rf = zeros(n, dtype=float64)  # Fast recharge Flux
        ...
        for t in range(n):
            ...
            if srn[t] > srmax:
                ...
                srn[t] = 0.0
                 ...
                 srn[t] = srn[t]
            rf[t] =  cp*rtot[t] # Fast recharge becareful with cr<1 in this case
            rfs[t] = (1-cp)*rtot[t] # Fast recharge to slow resarvoir
        ...
        return sr[:-1], rs, ea, rf, rfs, pe, cr,  alphat
