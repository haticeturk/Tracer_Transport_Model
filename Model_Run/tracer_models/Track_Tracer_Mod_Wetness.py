...
class Tracer_Mod(object):
    ...
    def Tracer_Tracking(self, prec, evap, temp, ConP, ConAvg, ConSuAvg,  p, alpha_Su, alpha_SG, **kwargs):
       ...
       #variable DEFINITION
        ...
        #Flux_RM is the total flux of the water out of the snow bucket (sum of RM_flux_MR  for each column
        ...
        sr, rs, ea, rff, rfs, pe, cr,  alphat  = self.get_root_zone_balance(pe, ep, srmax=p[0], lp=p[1], Rsmax=p[2], beta=p[3], dt=dt, cp = p[11], alpha_Su = alpha_Su)
        ...
        Qstot = Qs                                                            
        Qstot_Age_MR, Flux_Qstot, SSa_Age_MR, SSa_Conc_MR, SSa_Flux_MR, TB_Ss, Qstot_Conc_MR, Ql_Age_MR, Ql_Conc_MR, Ql_Flux_MR, Flux_Ql= self.Tracer_sampling_SG(Qin_Age_MR, Conc_in_MR, Qstot, Ql, Tr_length, alpha_SG, 1, x,  p[14], ConAvg)
  ...
  ### Fuctions to be used in the model
    ...
    def Rf_division(self, rf_Age_MR, rf_Conc_MR, Tr_length, cp, cn ):
        ...
        # fist divition of fast responce
    ...
    def Tracer_sampling_SG(self, Qin_Age_MR, Conc_in_MR, Qstot, Ql, Tr_length, alpha, Beta, x,  SSp, ConAvg):
            #outs 
            Qstot_Age_dist = np.zeros(Tr_length+1,  dtype=float64) #Qs Age Distribution  each time step
            ...
            Ql_Age_MR = np.zeros((Tr_length+1,Tr_length+1),  dtype=float64) #slow responce Age Distribution matrix
            ...
            Flux_Ql = np.zeros((Tr_length+1),  dtype=float64) #slow responce Flux
            ## SLow Responcew Flux
    ...
    def get_slow_res_balance(rs, Ka=0.05, Kp = 0.02, SSp = 5000.0, dt=1.0):
        ...
        for t in range(n):
          ...
          Qst[t] = ssan[t] - Stot_out[t] ## total loss and Qs slow responce
    ...
    def get_root_zone_balance(pe, ep, srmax=250.0, lp=0.25, Rsmax=2.0, beta=2.0, dt=1.0, cp = 0.8, alpha_Su = 1.0):
        ...
        rff = zeros(n, dtype=float64)  # Fast recharge Flux to fast reservoir and overland flow
        rtot = zeros(n, dtype=float64)  # Total Fast recharge Flux before division
        ...
        for t in range(n):
            ...
            rfs[t] = (1-cp)*rtot[t] # Fast recharge to slow reservoir
    ...
    def get_tracer_balance(self, prec, evap,  temp, ConP, ConAvg, ConSuAvg, p, Case, alpha_Su, alpha_SG, **kwargs):
    
        Age_MR, Flux_df, Storage_Age_MR, Storage_Flux_MR, TrB, WB_df = self.Tracer_Tracking(prec=prec, evap=evap, temp=temp, ConP =ConP, ConAvg = ConAvg,  ConSuAvg = ConSuAvg, p=p, alpha_Su = alpha_Su, alpha_SG =  alpha_SG, **kwargs)
