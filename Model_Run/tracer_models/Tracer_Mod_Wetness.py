...
class Tracer_Mod(object):
    ...
    def Tracer_Tracking(self, prec, evap, temp, ConP, ConAvg, ConSuAvg,  p,  **kwargs):
        ...
        #evap = Evaporation
        #temp = Temperature
        #ConP = Tracer  in precipitation
        #ConAvg = Average Concentration to be assigned to the groundwater passive storage
        #ConSuAvg = Average Concentration to be assigned to the root zone storage
        np.seterr(all="ignore")
    
        #Tracking  length  as lenght presipitataio as float
        Tr_length = prec.size
        x = [] # empty list for storage with multuple outfluxes to sample SAS function
        x=np.array(x)
        # time step
        dt  = 1.0
        ...
        #RM_Age_MR is is the Age matrix for the rain and melt water in the snow bucket (Tr_lengthxTr_length) by the end of tracking period
        #Conc_Age_MR is the Concentration matrix for the rain and melt water in the snow bucket(Tr_lengthxTr_length)  by the end of tracking period
        #RM_flux_MR is the Flux matrix for the rain and melt water in the snow bucket(Conc_Age_MR * RM_Age_MR) by the end of tracking period
        #Flux_RM is the total flux of the water out of the snow bucket ( for each time step)
        ...
        Pe_Age_MR, Pe_Flux_MR, Pe_Conc_MR, Ei_Age_MR, Ei_Flux_MR, Si_Age_MR, Si_Flux_MR,  Flux_Pe, Flux_Ei, TB_si = self.Tracer_sampling_Si(RM_Age_MR, Conc_Age_MR, pe, ei,  Tr_length, 1, 1, x)


        #Soil bucket
        ep = ep - ei  # Update potential evaporation after interception
        sr, rs, ea, rff, rfs, pe, cr,  alphat  = self.get_root_zone_balance(pe, ep, srmax=p[0], lp=p[1], Rsmax=p[2], beta=p[3], dt=dt, cp = p[11], alpha_Su = p[16])

        #Tracer balance in soil bucket
        #Time variable SAS_alpha parameter from root zone balance 

        rtot =  rff + rfs 
        rf_Age_MR, rf_Conc_MR, Flux_rf, ea_Age_MR, ea_Conc_MR, Flux_ea, rs_Age_MR, rs_Conc_MR, Flux_rs, Sr_Age_MR, Sr_Flux_MR, TB_Sr = self.Tracer_sampling_SR(Pe_Age_MR, Pe_Conc_MR, rtot, ea, rs, Tr_length, alphat, 1, x, p[0], ConSuAvg)
        ...
        sf, Qf, Qof = self.get_fast_res_balance(rfn, ep, sfmax=p[9], dt=dt,  Bf = p[13], kf=p[10])
        ...
        ConAvg = ConAvg
        Qin_Age_MR = rs_Age_MR +rfs_Age_MR # total recharge to groundwater
        Flux_Tot_MR = rs_Age_MR*rs_Conc_MR +rfs_Age_MR*rfs_Conc_MR # total flux to groundwater
        Conc_in_MR = Flux_Tot_MR/Qin_Age_MR #Concentration in the groundwater
     ...
     # water balance functions are below
    ...
    @staticmethod
    @njit
    def get_overland_flow(pr, rf, Ptresh =7.0,  cn = 0.4):
        n = pr.size
        # Create empty arrays to store the fluxes and states
        Qo = zeros(n, dtype=float64)  # Overland flow
        rfn = zeros(n, dtype=float64)  # fas responce flux to fast responce storage
        ...
        for t in range(n):
            ...
            if pr[t] > Ptresh:
                ...
                # else:
                #   Cn[t] = cn
                Qo[t]  = max(rf[t]*cn, 0.0)  # overland flow occur
                rfn[t]  = (1-cn)*rf[t]
            else:
                Qo[t]= 0.0
                rfn[t]  = rf[t]
    ...
    def get_slow_res_balance(rs, Ka=0.05, Kp = 0.02, SSp = 5000.0, dt=1.0):
        ...
        ssan = zeros(n + 1, dtype=float64)  # Updated Slow Storage
        Stot_out = zeros(n, dtype=float64)  # Total Slow Storage
        prop_QsQl = zeros(n, dtype=float64)  
        Qs = zeros(n, dtype=float64)  # QS runoff Flux
        Qst = zeros(n, dtype=float64)  # slow total runoff Flux
        Ql = zeros(n, dtype=float64)  # slow loss runoff Flux
        ssp[0] = SSp  # Set the initial system state
    ...
    @staticmethod
    @njit
    def get_fast_res_balance(rfn, ep, sfmax=20.0, dt=1.0, Bf = 1e-5, kf=0.02):
        n = rfn.size
        # Create empty arrays to store the fluxes and states
        sf = zeros(n + 1, dtype=float64)  # Fast Storage
        sfn = zeros(n + 1, dtype=float64)  # Updated Fast Storage
    ...
    @staticmethod
    @njit
    def get_root_zone_balance(pe, ep, srmax=250.0, lp=0.25, Rsmax=2.0, beta=2.0, dt=1.0, cp = 0.8, alpha_Su = 1.0):
        n = pe.size
        # Create empty arrays to store the fluxes and states
        sr = zeros(n + 1, dtype=float64)  # Root Zone Storage State
        sr[0] = 0.5 * srmax  # Set the initial system state to half-full
        srn = zeros(n + 1, dtype=float64) # Updated Root Zone Storage State
        ...
        alphat = zeros(n, dtype=float64) #alpha shape parameter
        for t in range(n):
            ...
            rfs[t] = (1-cp)*rtot[t] # Fast recharge to slow resorvoir
    ...
    def get_interception_balance(pr, ep, Imax=2.0, dt=1.0):
        n = pr.size
        si = zeros(n + 1, dtype=float64)  # Interception Storage 
        pe = zeros(n+ 1, dtype=float64)  # Effective precipitation Flux
        sm =  zeros(n, dtype=float64)  # updated interception storage
    ...
    def get_snow_balance(prec, temp, tt=0.0, fmelt=2.0):
        ...
        ssn = zeros(n + 1, dtype=float64)  # Updated Snow Storage
        ps = where(temp <= tt, prec, 0.0)  # Snowfall
        m = where(temp > tt, fmelt * (temp - tt), 0.0)  #Snowmelt
        # Snow bucket
        for t in range(n):
            ssn[t] = ss[t] + ps[t]
            if temp[t] > tt:
